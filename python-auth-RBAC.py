...
log = 0
failed_aut = 0
users = [["Kot", "admin", "admin"],
          ["Шут", "StayOut2026", "admin"],   #админы
          ["Stellar2026", "user123", "moderator"],   # модераторы
            ["Guest", "Guest", "users"],   # обычные юзеры
              ["hacker", "password123", "banned"]]   # баны
reports = []
print("Система авторизации приветствует вас! Для продолжения выберите один из перечисленных вариантов.")
while True:
    ...
    if var == 2:     
     ...
     if new_login.lower() == 'admin' or new_login.lower() == 'root':
        print("Логины 'admin' и 'root' зарезирвированы системой! Пожалуйста, смените логин. ")
        continue
        ...
        while attempts > 0:
         ...
         if role:
            ...
            if role == "admin":
               ...
               admin_key_check = input("Введите код доступа: ")
            ...
            elif role == "banned" or role == "ban":
               print("Связываемся с базой данных. Пожалуйста подождите...")
               time.sleep(2)
               print("Ошибка! Ваш аккаунт заблокирован по причине попытки взлома! Обратитесь к администрации сервиса.")
               rep = input("Хотите отправить репорт? (да/нет): ")
               if rep.lower() == "да":
                   text = input("Опишите проблему: ")
                   reports.append([login, text])
                   print("Заявка на обжалование блокировки отправлен! Администрация рассмотрит его.")
            break
